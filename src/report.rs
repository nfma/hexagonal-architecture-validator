...
pub fn render_text(report: &ValidationReport) -> String {
    ...
    match report.outcome {
        Outcome::Passed => {
            ...
            render_role_coverage(&mut output, report);
        }
        Outcome::Violations => {
            writeln!(
                output,
...
fn render_role_coverage(output: &mut String, report: &ValidationReport) {
    writeln!(
        output,
        "role coverage: {} classified module(s), {} unclassified module(s)",
        report.summary.classified_modules, report.summary.unclassified_modules
    )
    .expect("writing to a String cannot fail");
}
